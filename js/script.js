...
const agregarAlCarrito = (id) => {
    ...
    let espacioCards = '';
    carrito.forEach(function (seleccionado) {
        espacioCards += `<div width=100>
    <tr>
    <td>
...
document.getElementById('productosPop').innerHTML = espacioCards;
...
document.getElementById("total").innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cart" viewBox="0 0 16 16">
    ...
    totalCompra.innerHTML = `El total de tu compra es de $` + vistaTotal;
}
...
//VACIAR CARRITO
let btnVaciar = document.getElementById("btnPopVaciar");

btnVaciar.onclick = (id) => {
    const borrado = carrito.find((producto) => producto.id == id);
    carrito.splice(borrado);
    let espacioCards = '';
    carrito.forEach(function (borrado) {
        espacioCards += `<div width=100>
        <tr>
        <td>
        <img src="${borrado.foto}" width=100>
</td>
<td>${borrado.nombre}</td>
<td>$${borrado.precio}</td>
<td>
    <a href="#" onclick="borrarProducto(${borrado.id})" class="fas fa-times-circle">  </a>
</td>
</tr>
<div>`;
});
document.getElementById('productosPop').outerHTML = espacioCards;
totalCompra.innerHTML = `No hay productos seleccionados` ;
}
...
function eliminarProducto(id){
    ...
    carrito.splice(productoAEliminar);
}
